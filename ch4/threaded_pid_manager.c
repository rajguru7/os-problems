...
#include <pthread.h> //calloc
#include <time.h> //calloc
...
void* get_pid(void* param) {
    long pid = allocate_pid() + MIN_PID;
    printf("get id: %ld\n", pid);
    double ts = ((double)rand() / (double)RAND_MAX+1);
    sleep(((int)ts));
    release_pid(pid);
    printf("released get id: %ld\n",  pid);
    return NULL;
}
...
int main(int argc, char *argv[]) {
	allocate_map();
    srand(time(NULL));
    pthread_t threads[MAXLEN];
    for (int idx = 0; idx < MAXLEN; idx++) {
        pthread_create(&threads[idx], NULL, get_pid, NULL);
    }
    for (int idx = 0; idx < MAXLEN; idx++) {
