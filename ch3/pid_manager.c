...
int allocate_map() {
	...
	int size = (int)((total_bits+1)/64); //total bits must be for the whole tree
	bitarray = (uint64_t*)calloc(size, sizeof(uint64_t));//calloc is weird, even if size = 0, it initialises and reserves space to 0 for arbitrary size
	bitsize = size;
